...
## PM2 部署（Node Server 模式，推薦）

> 適用情境：VPS/裸機部署，由 Nginx 或 Caddy 反向代理到 Nuxt Nitro（預設 `127.0.0.1:8555`）。
...
- 參考：`deploy/pm2/.env.production.example`
- 建議在專案根目錄建立 `.env.production`（或你現有的 `.env` 正式檔）
...
> `pm2 startup` 會輸出一條需要 sudo 執行的命令，複製貼上執行即可。
...
pm2 status
pm2 logs hxm-oshop
pm2 restart hxm-oshop
```

PM2 設定檔：`deploy/pm2/ecosystem.config.cjs`  
預設 log 檔：`logs/pm2-out.log`、`logs/pm2-error.log`
